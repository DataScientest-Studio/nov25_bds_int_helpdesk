...
st.set_page_config(page_title="G – Settings", page_icon="⚙️", layout="wide")
...
for i, (page_file, trans_key, emoji) in enumerate(nav_items):
    ...
    with col1 if i % 2 == 0 else col2:
        new_state = st.toggle(f"{emoji} {page_name}", value=current_state, key=f"vis_{trans_key}")
...
if not IS_CLOUD:
    ...
    with col1:
        ...
        if st.button(e("🔄 ") + get_text('simulation_reset'), type="primary", width="stretch"):
            with st.spinner(get_text('resetting_database')):
                st.cache_data.clear()
                subprocess.run(["systemctl", "--user", "restart", "helpdesk-simulator"], capture_output=True)
    ...
    with col3:
        ...
        result = subprocess.run(["systemctl", "--user", "is-active", "helpdesk-simulator"], capture_output=True, text=True)
    ...
    for service in ['helpdesk-dashboard', 'helpdesk-api', 'helpdesk-simulator']:
...
with col2:
    st.markdown(f"""
    - **{get_text('project')}:** Employee Performance 5
    - **{get_text('version')}:** 5.0
