...
st.set_page_config(page_title="F – Export Center", page_icon="📥", layout="wide")
...
with tab1:
    ...
    with col1:
        ...
        if 'training' in data:
            df = data['training']
            st.write(f"**{len(df)} {get_text('employees')}**")
            csv = df.to_csv(index=False).encode('utf-8')
            st.download_button(
                e("📥 ") + get_text('download_csv'), csv,
    ...
    with col2:
        ...
        if 'ml' in data:
            ...
            csv = df.to_csv(index=False).encode('utf-8')
            st.download_button(
                e("📥 ") + f"{get_text('ml_dataset')} (CSV)", csv,
    ...
    with col1:
        ...
        if 'nlp' in data:
            ...
            csv = df.to_csv(index=False).encode('utf-8')
            st.download_button(
                e("📥 ") + f"{get_text('nlp_features')} (CSV)", csv,
    ...
    with col2:
        ...
        if 'workflow' in data:
            ...
            csv = df.to_csv(index=False).encode('utf-8')
            st.download_button(
                e("📥 ") + f"{get_text('workflow_analysis')} (CSV)", csv,
